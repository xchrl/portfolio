    ...
    <section id="about">
      <main className="container px-4 lg:px-8 xl:px-16 mx-auto my-8 xl:mb-0 flex flex-col lg:flex-row items-center gap-8">
        <div className="lg:w-1/2 flex justify-center xl:self-end items-center">
          <div className="xl:hidden size-50 lg:size-80 overflow-hidden rounded-full bg-foreground/5 p-4 border-2 border-muted-foreground">
            <img src={silhouette} alt="Silhouette" />
          </div>
          <img
            src={silhouette}
            alt="Silhouette"
            className="hidden xl:block w-6/10 self-end"
          />
        </div>
        <div className="flex lg:w-1/2 flex-col gap-4 text-center mb-8">
          <h3 className="text-2xl sm:text-3xl font-bold">{t("title")}</h3>
          <p className="lg:text-lg">{t("description")}</p>
          <p className="lg:text-lg">{t("description_2")}</p>
